...
static std::shared_ptr<Publicatie> creeazaPubL (const std::string& line,const AppState& app) {
    ...
    auto cautaAutor = [&](const int id) {
        for (auto& a : app.autor)
            if (a->getidAutor() == id)
                return a;
        throw std::runtime_error("Autor inexistent!");
    };
    if (tip == "Carte") {
        return std::make_shared<Carte>(
            v[2], cautaAutor(std::stoi(v[8])),
            std::stoi(v[4]), v[5],
            v[1], std::stod(v[3]),
            std::stoi(v[6]), v[7]
        );
    ...
    if (tip == "Manual") {
        return std::make_shared<Manual>(
            v[2], cautaAutor(std::stoi(v[8])),
            std::stod(v[3]), std::stoi(v[4]), v[5],
            v[1], std::stoi(v[6]), v[7],
            v[9], std::stoi(v[10])
        );
    }

    if (tip == "Carte_stiintifica") {
        return std::make_shared<CarteStiintifica>(
            v[2], cautaAutor(std::stoi(v[8])),
            std::stoi(v[4]), v[5], v[1],
            std::stod(v[3]), std::stoi(v[6]), v[7],
            v[9], v[10], std::stoi(v[11]), v[12] == "1"
        );
    }

    if (tip == "Revista") {
        return std::make_shared<Revista>(
            v[2], std::stoi(v[4]), v[5], std::stoi(v[6]),
            std::stod(v[3]), v[8], std::stoi(v[9]),
            v[10] == "1", v[11], v[1], v[7]
        );
    }
...
void LoadService::salveazaClienti(const AppState &app) {
    ...
    for (const auto& c : app.clienti) {
        out << c->getEmail() << "|"
            ...
            << (c->getComandaActiva() ? c->getComandaActiva()->getId() : -1)
            << "\n";
    }
}
void LoadService::incarcaClienti(AppState& app) {
    ...
    while (std::getline(in, line)) {
        ...
        // comanda activa va fi legata DUPA ce incarcam comenzile
        client->setIdComandaActiva(std::stoi(v[11]));
